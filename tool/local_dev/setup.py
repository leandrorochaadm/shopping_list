...
PG_PORT = 5432
POSTGREST_PORT = 3000
# The same password bootstrap.sql gives the `authenticator` role. It only ever
# reaches a loopback socket, and postgrest.conf is written 600.
AUTHENTICATOR_PASSWORD = "postgrest_local"
...
def start_postgres_hint() -> str:
    """How to start the server on THIS machine.

    The project has been developed on both macOS (Homebrew) and Debian
    (apt + systemd), and a hint naming the wrong package manager is worse than
    no hint: it sends the reader looking for a `brew` that is not there.
    """
    if sys.platform == "darwin":
        return "brew services start postgresql@17"
...
def require_postgres() -> None:
    ...
    if probe.returncode != 0:
        sys.exit(
            ...
            "Start it with:\n"
            f"  {start_postgres_hint()}"
        )
    require_login_role()


def require_login_role() -> None:
    """A role named after the OS user -- which apt does not create, and brew does.

    Homebrew initializes the cluster as the developer, so `psql` just works.
    The Debian package initializes it as `postgres`, and every command below
    dies with `FATAL: role "<user>" does not exist` -- an error that says
    nothing about the one-line fix.
    """
    probe = subprocess.run(
        ["psql", "-tAc", "select 1", "-d", "postgres"], capture_output=True, text=True
    )
    if probe.returncode == 0:
        return
    if "does not exist" not in probe.stderr:
        print(probe.stderr.strip(), file=sys.stderr)
        sys.exit("cannot connect to postgres")
    user = getpass.getuser()
    sys.exit(
        f'postgres has no role named "{user}", so psql cannot connect.\n'
        "Create it once with:\n"
...
#
# The password is the one bootstrap.sql gives `authenticator`, and it is here
# because the URI is TCP: a Homebrew cluster trusts local TCP, the Debian one
# asks for scram-sha-256, and PostgREST then dies with "no password supplied".
# 127.0.0.1 and not `localhost`: the name resolves to ::1 first, which is a
# different pg_hba line and a different failure on the same machine.
db-uri = "postgres://authenticator:{AUTHENTICATOR_PASSWORD}@127.0.0.1:{PG_PORT}/{DB_NAME}"
db-schemas = "public"
...
{{
	# The apt package leaves a caddy service running, and it holds the default
	# admin port 2019. Ours has no admin API to offer, so it just steps aside --
	# without this the second caddy refuses to start with "address already in use".
	admin off
}}
